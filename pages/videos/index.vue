...
const { data: featuredVideos } = await useAsyncData('featured-videos',
...
// Fetch recent videos
const { data: recentVideos } = await useAsyncData('recent-videos',
  () => queryCollection('videos')
    .order('date', 'DESC')
    .limit(8)
    .all(),
...
const { data: conferenceVideos } = await useAsyncData('conference-videos',
  () => queryCollection('videos')
...
const { data: liveStreamVideos } = await useAsyncData('livestream-videos',
  () => queryCollection('videos')
...
<template>
  <PageLayout :title="title" :description="description" :section="section">
    ...
    <section v-if="featuredVideos && featuredVideos.length > 0" class="mb-16">
      <h2 class="text-2xl font-bold text-gray-900 mb-6">Featured Videos</h2>
      <VideoList :list="featuredVideos" />
    </section>
    
    <!-- Recent Videos Section -->
    <section class="mb-16">
      <h2 class="text-2xl font-bold text-gray-900 mb-6">Recent Videos</h2>
    ...
    <section v-if="conferenceVideos && conferenceVideos.length > 0" class="mb-16">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-900">Conference Talks</h2>
        <NuxtLink 
          to="/videos/tags/conference" 
    ...
    <section v-if="liveStreamVideos && liveStreamVideos.length > 0" class="mb-16">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-900">Live Streams</h2>
        <NuxtLink 
