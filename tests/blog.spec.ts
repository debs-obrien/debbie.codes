...
test('blog has a heading', async ({ page }) => {
  await expect(page.getByRole('heading', { name: 'Testing a Sites Color Mode with Playwright' }))
    .toBeVisible();
});

test('blog has a date', async ({ page }) => {
  await expect(page.getByText('September 3, 2022'))
    .toBeVisible();
});

test('blog has an image', async ({ page }) => {
  await expect(page.getByRole('img', { name: 'Testing a Sites Color Mode with Playwright' }))
    .toBeVisible();
});

test('blog has a summary', async ({ page }) => {
  await expect(page.getByText('My site has a color mode option to change from light theme to dark theme or sepi')).toBeVisible();
});
...
test('blog has a tag that links to tag page', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Blog Posts on testing' }))
    .toBeVisible();
});

test('blog has content', async ({ page }) => {
  await expect(page.getByText('My website uses the Nuxt color mode module to allow the user to change the theme'))
    .toBeVisible();
});

test('blog has a working TOC', async ({ page }) => {
  await expect(page.getByRole('heading', { name: 'Table of contents' }))
    .toBeVisible();
  ...
  await expect(page.getByRole('heading', { name: 'Setting the Base' })
    .getByRole('link', { name: 'Setting the Base' }))
    .toBeVisible();
  ...
  await expect(page.getByRole('heading', { name: 'Checking it works' })
    .getByRole('link', { name: 'Checking it works' }))
    .toBeVisible();
});
